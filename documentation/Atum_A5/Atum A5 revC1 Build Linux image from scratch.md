# Atum A5 revC1: Build Linux image from scratch
...
git clone -b atuma5-revC1_v1.0 https://github.com/terasic/arm-trusted-firmware arm-trusted-firmware
...
*   \$TOP\_FOLDER/arm-trusted-firmware/build/agilex5/release/bl31.bin (to be used for u-boot.itb generation)
...
git clone -b atuma5-revC1_v1.0 https://github.com/terasic/u-boot-socfpga u-boot-socfpga
cd u-boot-socfpga
make mrproper
make socfpga_agilex5_atuma5_defconfig
# link to ATF
...
*   \$TOP\_FOLDER/u-boot-socfpga/u-boot.itb
*   \$TOP\_FOLDER/u-boot-socfpga/spl/u-boot-spl-dtb.hex
...
vi uboot_script.its # content see bloew
...
>         fatload mmc 0:1 ${fdt_addr_r} socfpga_agilex5_atuma5.dtb;
...
*   \$TOP\_FOLDER/uboot-script/boot.scr.uimg
...
cd $TOP_FOLDER
rm -rf linux-socfpga
git clone -b atuma5-revC1_v1.1 https://github.com/terasic/linux-socfpga linux-socfpga
cd linux-socfpga
cp atuma5-revC1-2.5G.config .config
make -j $(nproc) Image modules dtbs
```

The following files are created:

*   \$TOP\_FOLDER/linux-socfpga/arch/arm64/boot/Image
*   \$TOPFOLDER/linux-socfpga/arch/arm64/boot/dts/intel/socfpga\_agilex5\_atuma5.dtb
...
cp $TOP_FOLDER/linux-socfpga/arch/arm64/boot/dts/intel/socfpga_agilex5_atuma5.dtb .
...
```shell
cd $TOP_FOLDER
sudo rm -rf emmc && mkdir emmc && cd emmc
wget https://releases.rocketboards.org/release/2020.11/gsrd/tools/make_sdimage_p3.py
# remove mkfs.fat parameter which has some issues on Ubuntu 22.04
sed -i 's/\"\-F 32\",//g' make_sdimage_p3.py
chmod +x make_sdimage_p3.py
...
cp $TOP_FOLDER/u-boot-socfpga/u-boot.itb .
cp $TOP_FOLDER/linux-socfpga/arch/arm64/boot/Image .
cp $TOP_FOLDER/linux-socfpga/arch/arm64/boot/dts/intel/socfpga_agilex5_atuma5_emmc.dtb ./socfpga_agilex5_atuma5.dtb
cp $TOP_FOLDER/uboot-script/boot.scr.uimg .
cd ..
...
sudo cp -a  $TOP_FOLDER/rootfs/* .
cd $TOP_FOLDER/linux-socfpga
sudo make -j $(nproc) modules_install INSTALL_MOD_PATH=$TOP_FOLDER/emmc/rootfs
...
1.  Make sure Quartus Pro 25.3.1 is installed
2.  Go to GHRD project directory (copy from Atum A5 System CD)
3.  Copy \$TOP\_FOLDER/u-boot-socfpga/spl/u-boot-spl-dtb.hex to software/u-boot/spl/u-boot-spl-dtb.hex
4.  Execute sof\_with\_hps.bat to generate output\_files/golden\_top\_hps.sof
5.  Execute sof\_to\_jic.bat to generate output\_files/golden\_top\_hps.jic
