...
1.  Split the eMMC image
    ...
    ```shell
    unzip comet_a65_revB_sdcard_console_v1.3.zip
    export source_img=comet_a65_revB_sdcard_console_v1.3.img
...
6.  Execute the following commands to program the eMMC:
    ...
    fatls usb 0:1 # 0:1 indicates the first partition of the first USB device. It will list the found filenames, e.g.,comet_a65_revB_sdcard_console_v1.3.img.part00
    ...
    env set image_file comet_a65_revB_sdcard_console_v1.3.img; # Replace the image name if a newer version is used
    # Start loading files
